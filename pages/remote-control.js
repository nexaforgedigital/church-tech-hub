...
export default function RemoteControl() {
  ...
  const [lastCommand, setLastCommand] = useState(''); // For debugging
  ...
  const handleCommand = async (command) => {
    ...
    setLastCommand(command); // For visual feedback
  ...
  return (
    <div className="h-screen flex flex-col bg-gradient-to-br from-blue-600 via-purple-600 to-pink-600 text-white">
      ...
      <div className="p-4 md:p-6 bg-black/30 backdrop-blur-md flex-shrink-0">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl md:text-2xl font-bold">🎮 Remote Control</h1>
            <p className="text-xs md:text-sm opacity-75 flex items-center gap-2">
              {connected ? (
                <>
          ...
          <div className="text-right">
            <div className="text-2xl md:text-3xl font-bold">{currentSlide + 1}</div>
            <div className="text-xs md:text-sm opacity-75">of {totalSlides}</div>
          </div>
        </div>
        
        {/* Debug Info - Remove after testing */}
        {lastCommand && (
          <div className="mt-2 text-xs bg-green-500/30 rounded px-2 py-1 text-center">
            Sent: {lastCommand}
          </div>
        )}
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-4 md:p-8 overflow-hidden">
        <div className="text-center">
          ...
          <p className="text-xl md:text-3xl font-bold mb-2 md:mb-4">Swipe to Navigate</p>
          <div className="space-y-1 md:space-y-2 text-sm md:text-lg opacity-90">
            <p>← Swipe right for previous</p>
            <p>Swipe left for next →</p>
          </div>
          
          {!connected && totalSlides === 0 && (
            <div className="mt-6 md:mt-8 bg-yellow-500/20 backdrop-blur rounded-xl p-4 md:p-6 max-w-md mx-auto">
              <div className="text-3xl md:text-4xl mb-2 md:mb-3">⏳</div>
              <p className="text-base md:text-lg font-semibold mb-1 md:mb-2">Waiting for presenter...</p>
              <p className="text-xs md:text-sm opacity-75">Make sure presenter control is open on your computer</p>
            </div>
          )}

          {connected && (
            <div className="mt-6 md:mt-8 bg-green-500/20 backdrop-blur rounded-xl p-3 md:p-4 max-w-md mx-auto">
              <p className="text-xs md:text-sm">✅ Successfully connected to presentation!</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="p-4 md:p-6 bg-black/30 backdrop-blur-md flex-shrink-0">
        ...
        <div className="grid grid-cols-3 gap-2 md:gap-3 mb-3 md:mb-4">
          <button
            onClick={goToFirst}
            disabled={!connected}
            className="bg-white/20 hover:bg-white/30 active:bg-white/40 disabled:opacity-30 p-3 md:p-5 rounded-lg md:rounded-xl transition active:scale-95 flex flex-col items-center justify-center"
          >
            <Home size={20} className="md:hidden" />
            <Home size={28} className="hidden md:block" />
            <p className="text-xs mt-1 md:mt-2">First</p>
          </button>
          <button
          ...
          >
            ...
            <Grid size={28} className="hidden md:block" />
            <p className="text-xs mt-1 md:mt-2">Grid</p>
          </button>
          <button
            onClick={goToLast}
            disabled={!connected}
            className="bg-white/20 hover:bg-white/30 active:bg-white/40 disabled:opacity-30 p-3 md:p-5 rounded-lg md:rounded-xl transition active:scale-95 flex flex-col items-center justify-center"
          >
            <Square size={20} className="md:hidden" />
            <Square size={28} className="hidden md:block" />
            <p className="text-xs mt-1 md:mt-2">Last</p>
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 md:gap-4">
          <button
            ...
            className={`bg-white/20 hover:bg-white/30 active:bg-white/40 disabled:opacity-30 p-6 md:p-8 rounded-xl transition active:scale-95 flex flex-col items-center justify-center ${
          ...
          >
            ...
            <p className="text-base md:text-lg font-bold mt-2">Previous</p>
          </button>
          <button
            onClick={nextSlide}
          ...
          >
            ...
            <p className="text-base md:text-lg font-bold mt-2">Next</p>
          </button>
        </div>

        {/* Progress Bar */}
        <div className="mt-3 md:mt-4 bg-white/10 rounded-lg p-3 md:p-4">
